...
Retryable failures exit `75` and persist their sanitized class in `retry.json`; they are no longer reported to systemd as successful runs. After `VK_MAX_PUBLISH_RETRIES` attempts (default `12`), the job moves to `failed` so a single incompatible item cannot block the rest of the queue. A later administrative `requeue-failed` clears the retry state. If the consumer is killed while a job is in `processing`, the next locked run reconciles it with durable publication receipts before deciding between `done` and `pending`.

The shared `recent-tracks.json` is a full least-recently-used history, not an eight-post window. Keep `VK_TRACK_ROTATION_SIZE` equal to the VOID allowlist size (currently `149`) and `VK_MUSIC_TRACKS_FILE` pointed at that exact catalog in the consumer environment. Every VOID job must use a key from that catalog; Naz retains its independent smaller-catalog policy. On the first full-history consumer run, confirmed historical jobs are backfilled while the known legacy last-eight suffix remains authoritative. The v3 marker keeps a recoverable LRU snapshot and receipt watermark, so a missing or old-consumer-truncated history is restored before publication. An early repeat or off-catalog VOID job is moved to `failed` before the browser opens the composer.
